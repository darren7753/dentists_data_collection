...
url = "https://yankes.kemkes.go.id/rumahsakit/cari?propinsi=&kabkota=&namapelayanan=&namarumahsakit=gigi"
driver.get(url)
...
def scrape_data():
    name_elements = driver.find_element(By.CSS_SELECTOR, 'h4.mb-0').text
    name = name_elements.split("\n")[0]
    address = name_elements.split("\n")[1]
    phone = driver.find_elements(By.XPATH, '//font[@class="text-info"]')[4].text
...
df.to_excel("Data/data_fourth_link.xlsx", index=False)